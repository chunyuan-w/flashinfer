...
void rmsnorm_kernel_impl(
  ...
  at::parallel_for(0, batch_size, 0, [&](int begin, int end) {
    for (int i = begin; i < end; ++i) {
      float var = at::vec::map_reduce_all<scalar_t>(
            [](Vec x) { return x * x; },
            [](Vec x, Vec y) { return x + y; },
            input + i * hidden_size,
            hidden_size);
      float rsqrt_var = float(1) / std::sqrt(var / hidden_size + eps);
...
#pragma GCC unroll 4
      for (d = 0; d <= hidden_size - kVecSize; d += kVecSize) {
        ...
        bVec out_bvec = at::vec::convert_from_float<scalar_t>(x_fvec0, x_fvec1);
...
#pragma GCC unroll 4
      ...
      sum_val += at::vec::vec_reduce_all([](fVec& x, fVec& y) { return x + y; }, sum_fvec);
